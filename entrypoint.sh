...
DEFAULT_JAVA_OPTS="${DEFAULT_JAVA_OPTS} -Djava.net.preferIPv4Stack=true -Djava.net.preferIPv6Addresses=false -Dconfig.file=/conf/application.conf"
...
echo "Running $@"
exec "$JAVA" ${DEFAULT_JAVA_OPTS} ${JAVA_OPTS} -jar /app/gatling_cassandra_timeslice "$@"
exit 1
